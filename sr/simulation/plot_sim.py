...
# stage4:
#filenames_in = [r"C:\Users\chwidmer\Documents\results\synthetic\seed3_fixed.pickle", r"C:\Users\chwidmer\Documents\results\synthetic\seed9_fixed_delta.pickle", r"C:\Users\chwidmer\Documents\results\synthetic\seed2_fix.pickle"]
filenames_in = [r"C:\Users\chwidmer\Documents\results\synthetic\hope.pickle",
                r"C:\Users\chwidmer\Documents\results\synthetic\finalrun_3.pickle",
                r"C:\Users\chwidmer\Documents\results\synthetic\finalrun_4.pickle",]
...
methodmap={
 "linreg_0":"Linreg",
 "linreg_auto": r"Linreg + PCs",
 "linreg": r"Linreg + PCpheno",
 "lmm_ins_0":"LMM(all)",
 "lmm_ins_auto": r"LMM(all) + PCs", 
 "lmm_oos_auto": r"Standard PCg oos",
 "lmm_ins": r"LMM(all) + PCpheno",
 "insample_0":"LMM(select)",
 "insample_auto": r"LMM(select) + PCs",
 "insample": r"LMM(select) + PCpheno",
 "lmm_fs_cond_full_auto": r"LMM(all+select)",
...
def PlotForOneMethodsList():
    ...
    for i_f,f in enumerate(num_causal):
        ...
        num_set = sum(k)
        tmp_name = "stage1_bycausals_num%i" % num_set
        try:
            plot_ind(ind=k,val=f,name=tmp_name,styles=styles)
        except:
            pass
    ...
    for i_f,f in enumerate(num_causal):
        ...
        k = np.bitwise_and(k, i_sib==0)
        num_set = sum(k)
        tmp_name = "stage2_hv0.0_bycausals_num%i" % num_set
        try:
            plot_ind(ind=k,val=f,name=tmp_name,styles=styles)
        except:
            pass
    ...
    for i_f,f in enumerate(num_causal):
        k=i_ncausal==i_f
        k = np.bitwise_and(k, i_fst > 0)
        k = np.bitwise_and(k, i_varh==1)
        k = np.bitwise_and(k, i_sib==0)
        num_set = sum(k)
        tmp_name = "stage2_hv0.3_bycausals_num%i" % num_set
        try:
            plot_ind(ind=k,val=f,name=tmp_name,styles=styles)
        except:
            pass
    ...
    if 1:#new all plot
        ...
        num_set = sum(k)
        try:
            tmp_name = "stage2_hv0.0_num%i" % num_set
    ...
    if 1:#new all plot
        ...
        num_set = sum(k)
        try:
            tmp_name = "stage2_hv0.3_num%i" % num_set
    ...
    for i_f,f in enumerate(num_causal):
        ...
        k = np.bitwise_and(k, i_sib>0)
        num_set = sum(k)
        tmp_name = "stage3_hv0.0_bycausals_num%i" % num_set
        try:
            plot_ind(ind=k,val=f,name=tmp_name,styles=styles)
        except:
            pass
    ...
    for i_f,f in enumerate(num_causal):
        ...
        k = np.bitwise_and(k, i_sib>0)
        num_set = sum(k)
        tmp_name = "stage3_hv0.3_bycausals_num%i" % num_set
        try:
            plot_ind(ind=k,val=f,name=tmp_name,styles=styles)
        except:
            pass
    ...
    if 1:#new all plot
        ...
        num_set = sum(k)
        try:
            tmp_name = "stage3_hv0.0_num%i" % num_set
    ...
    if 1:#new all plot
        ...
        num_set = sum(k)
        try:
            tmp_name = "stage3_hv0.3_num%i" % num_set
    ...
    for i_f,f in enumerate(num_causal):
        ...
        k = np.bitwise_and(k, i_sib>0)
        num_set = sum(k)
        tmp_name = "stage4_hv0.0_bycausals_num%i" % num_set
        try:
            plot_ind(ind=k,val=f,name=tmp_name,styles=styles)
        except:
            pass
    ...
    if 1:#new all plot
        ...
        num_set = sum(k)
        try:
            tmp_name = "stage4_hv0.0_num%i" % num_set
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        if 0:
            ...
            FSTs = FSTs = np.array([0.005, 0.01, 0.05, 0.1])
            ...
            FSTs = np.array([0.0, 0.005, 0.01, 0.05, 0.1])
            fracSibs=np.array([0.0, 0.5, 0.6, 0.7, 0.8, 0.9])
            h2s = np.arange(0.1, 0.7, 0.1) #! this is a range!  6 entries
            var_hidden = np.array([0.0, 0.3]) # ! this is a range! 4 entries
            num_causal = np.array([10, 50, 100, 500, 1000])
...
#        #methods = ["oosample_auto","lmm_ins_auto", "linreg_auto"]
        ...
        for folder_name, methods in [
                                                ...
                                                #["auto_tc", ["linreg_auto", "lmm_ins_auto", "insample_auto", "lmm_fs_cond_full_auto", "lmm_true_causals_auto"]]
                                                ["auto", ["linreg_auto", "lmm_ins_auto", "insample_auto", "lmm_fs_cond_full_auto"]]
