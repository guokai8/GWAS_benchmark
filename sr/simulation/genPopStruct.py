...
def generateIndividualPhenotype(options,individualMafCounts, numCausativeSNPs, snpEffectSizes, epsilon, freqs, covars):	
    ...
    ------------------------------------------------------------------------------- 
    freqs:      2 dim array of allele frequencies (What are the frequencies?)
    covars :    covariates
    options:    structure
...
def generateCovars(freq, numAlleles):
...
def findThreshold(diseasePrevalence, sampleSize, numCausativeSNPs, snpEffectSizes, epsilon, freqs, realFreqs):
	...
	for i in range(sampleSize):
		...
		#Add covariates	
		covars = []
		if (lastObservedSnpIndex >= 0): covars = generateCovars(freqs[lastObservedSnpIndex], individualMafCounts[lastObservedSnpIndex])
			
		
		individualLiability = generateIndividualPhenotype(options,individualMafCounts, numCausativeSNPs, snpEffectSizes, epsilon, realFreqs, covars)
...
if __name__ == "__main__":
    ...
    if (options.effStd == 0.0): #fixed effects
        snpEffectSizes = [1.0 for x in range(numCausativeSNPs)]
        try:
            snpEffectSizes[0] = options.snpCoeff
            for i in range(numCausativeSNPs-options.numHiddenSnps, numCausativeSNPs): snpEffectSizes[i] = hidCoeff
        except: pass
    else:                       #random effects
        snpEffectSizes = [random.gauss(0.0, options.effStd) for x in range(numCausativeSNPs)]
    lastObservedSnpIndex = options.csnps-1 - options.numHiddenSnps
    if (options.covarStd>1e-20): snpEffectSizes[lastObservedSnpIndex] = 0.0
    ...
    for i in xrange(1,min(options.numRare+1, options.csnps-1)): ancestralFreqs[i] = options.rareFreq
    ...
    while (casesCounter < numCases or controlsCounter < numControls):
        for popIndex in range(len(fst)):
            ...
            #Add covariates
            covars = []
            if (lastObservedSnpIndex >= 0): covars = generateCovars(freqs[popIndex][lastObservedSnpIndex], individualMafCounts[lastObservedSnpIndex])
